  ...
  return selectedUser && (
    ...
    ${selectedUser ? 'max-md:hidden' : ''}`}>
      ...
      <div className='pt-16 flex flex-col items-center gap-2 text-xs font-light mx-auto'>
        <img alt='' src={selectedUser?.profilePic || assets.avatar_icon} className='w-20 aspect-[1/1] rounded-full' />
      ...
      <hr className='border-[#fff50 my-4' />
        ...
        <div className='mt-2 max-h-[200px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
          {msgImages.map((url, index) => (
            <div onClick={() => window.open(url)} key={index} className='cursor-pointer rounded'>
              <img alt='' src={url} className='h-full rounded-md' />
            </div>
          ))}
        </div>
      </div>
